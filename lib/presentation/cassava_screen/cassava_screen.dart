import '../cassava_screen/widgets/cassava_item_widget.dart';
import 'bloc/cassava_bloc.dart';
import 'models/cassava_item_model.dart';
import 'models/cassava_model.dart';
...
class CassavaScreen extends StatelessWidget {
  CassavaScreen({Key? key})
      : super(
          key: key,
        );
  ...
  static Widget builder(BuildContext context) {
    return BlocProvider<CassavaBloc>(
      create: (context) => CassavaBloc(CassavaState(
        cassavaModelObj: CassavaModel(),
      ))
        ..add(CassavaInitialEvent()),
      child: CassavaScreen(),
    );
  }
                    ...
                    CustomAppBar(
                      ...
                      title: SizedBox(
                        ...
                        child: Stack(
                          alignment: Alignment.topLeft,
                          children: [
                            BlocSelector<CassavaBloc, CassavaState,
                    ...
                    children: [
                      ...
                      Align(
                        ...
                        child: Padding(
                          ...
                          child: BlocSelector<CassavaBloc, CassavaState,
                              CassavaModel?>(
                            selector: (state) => state.cassavaModelObj,
                            builder: (context, cassavaModelObj) {
                              return ListView.separated(
                                ...
                                itemCount:
                                    cassavaModelObj?.cassavaItemList.length ??
                                        0,
                                itemBuilder: (context, index) {
                                  CassavaItemModel model =
                                      cassavaModelObj?.cassavaItemList[index] ??
                                          CassavaItemModel();
                                  return CassavaItemWidget(
                                    model,
                                  );
                                },
                              );
